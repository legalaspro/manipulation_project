...
from detection_interfaces.msg import DetectedSurfaces, DetectedObjects
...
class ObjectDetection(Node):
    ...
    def callback(self, msg: PointCloud2) -> None:
        ...
        try:
            ...
            # # Clustering: Identify clusters corresponding to objects placed on top of table
            object_clusters, object_centroids, object_dimensions = self.extract_clusters(filtered_cloud_objects, "Object")

            # # Publish the detected surface surface clusters as markers
    ...
    def compute_bounds(self, cloud: pcl.PointCloud) -> None:
        """Compute and log min/max bounds for X, Y, Z axes in base_link frame."""
        points = np.asarray(cloud)
        valid_points = points[~np.isnan(points).any(axis=1)]

        if valid_points.size == 0:
            self.get_logger().warning('No valid points in cloud (all NaN or empty)')
            
        min_points = np.min(valid_points, axis=0)
        max_points = np.max(valid_points, axis=0)
        self.get_logger().info("Point Cloud Bounds in base_link frame:")
        self.get_logger().info(f'Min X: {min_points[0]}  Max X: {max_points[0]}')
        self.get_logger().info(f'Min Y: {min_points[1]}  Max Y: {max_points[1]}') 
        self.get_logger().info(f'Min Z: {min_points[2]}  Max Z: {max_points[2]}')
    ...
    def filter_cloud(self, cloud: pcl.PointCloud, min_y: float, min_height: float, max_height: float) -> Union[pcl.PointCloud, None]:
        """Filter cloud"""
        try:
            # Filter Y axis (min only; use float('inf') for no max if needed)
            passthrough_y = cloud.make_passthrough_filter()
            passthrough_y.set_filter_field_name('y')
            passthrough_y.set_filter_limits(min_y, float('inf'))  # Min Y, no upper bound
            filtered_y = passthrough_y.filter()

            # Filter Z axis
            passthrough_z = filtered_y.make_passthrough_filter()
            passthrough_z.set_filter_field_name('z')
            passthrough_z.set_filter_limits(min_height, max_height)
            filtered_cloud = passthrough_z.filter()
    ...
    def extract_clusters(self, cloud: pcl.PointCloud, cluster_type: str) -> Tuple[List[pcl.PointCloud], List[List[float]], List[List[float]]]:
        """Extracts clusters corresponding to object from the point cloud"""
        ...
        # Initialize lists to store object clusters, centroids, and dimensions
        object_clusters = []
        cluster_centroids = []
        cluster_dimensions = []
        ...
        # Return the filtered object clusters, centroids and cluster dimensions
    ...
    def pub_surface_marker(self, surface_centroids: List[List[float]], surface_dimensions: List[List[float]]) -> None:
        """Publishes the detected surface plane as cube markers"""
    ...
    def pub_object_marker(self, object_centroids: List[List[float]], object_dimensions: List[List[float]]) -> None:
        ...
        for idx, (centroid, dimensions) in enumerate(zip(object_centroids, object_dimensions)):
            ...
            marker.pose.position.y = float(centroid[1]) + offset_y
